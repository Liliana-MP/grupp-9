...
async function validate_product(form) {
  ...
  const producer = data.company.find(
    (item) => item.name.toUpperCase() == $producer.toUpperCase()
  );
  const category = data.category.find(
    (item) => item.name.toUpperCase() == $category.toUpperCase()
  );
  ...
  else if (isNaN($quantity)) errorTxt.text("* Bara nummer i antal fältet");
  else if (isNaN($price)) errorTxt.text("* Bara nummer i pris fältet");
  else if ($quantity < 0) errorTxt.text("* Antal kan inte bli mindre än 0");
  else if ($price <= 0) errorTxt.text("* Pris måste vara över 0");
  else if ($description == "") errorTxt.text("* Fyll i beskrivningen");
  else {
    let reader = new FileReader();
    let image = $("#product-image").prop("files")[0];
    reader.addEventListener("load", () => {
      // funktion för att skicka produkt till db
...
async function render_product() {
  ...
  if ($product == "") errorTxt.text("* Fyll i fältet");
  else {
    const product = data.product.find(
      (item) => item.name.toUpperCase() == $product.toUpperCase()
    );

    if (product !== undefined) {
      const producer = await data.company.find(
        (item) => item.id == product.companyid
      );
      const category = await data.category.find(
...
async function remove_product(form) {
  ...
  const product = data.product.find(
    (item) => item.name.toUpperCase() == $product.val().toUpperCase()
  );
  if ($product.val() == "") errorTxt.text("* Fyll i fältet");
  else if (product == undefined) errorTxt.text("* Produkten finns inte");
  else {
    alert("borta");
    $(form).parent().submit();
...
async function validate_category(form) {
  ...
  const category = data.category.find(
    (item) => item.name.toUpperCase() == $old.toUpperCase()
  );
  const newCategory = data.category.find(
    (item) => item.name.toUpperCase() == $new.toUpperCase()
  );

  if ($old == "" && newCategory == undefined) {
    // funktion för att spara kategori till db
    form.submit();
  } else if ($new == "") errorTxt.text("* Fyll i alla fälten korrekt");
...
async function render_category() {
  ...
  if ($input == "") errorTxt.text("* Fyll i fältet");
  else {
    const category = data.category.find(
      (item) => item.name.toUpperCase() == $input.toUpperCase()
    );
...
async function remove_category(form) {
  ...
  const category = data.category.find(
    (item) => item.name.toUpperCase() == $category.val().toUpperCase()
  );
  if ($category.val() == "") errorTxt.text("* Fyll i fältet");
  else if (category == undefined) errorTxt.text("* Kategorin finns inte");
  else {
    alert("borta");
    $(form).parent().submit();
...
async function validate_company(form) {
  ...
  const newCompany = data.company.find(
    (item) => item.name.toUpperCase() == $new.toUpperCase()
  );
...
async function remove_company(form) {
  ...
  const category = data.company.find(
    (item) => item.name.toUpperCase() == $company.val().toUpperCase()
  );
  if ($company.val() == "") errorTxt.text("* Fyll i fältet");
  else if (category == undefined) errorTxt.text("* Tillverkaren finns inte");
  else {
    alert("borta");
    $(form).parent().submit();
